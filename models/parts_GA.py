...
class Convolutional1D:
   ...
   def build(self, input_layer):
      ...
      self.weights = weight_variable(weight_shape, name='w_'+self.name)
      self.layer = tf.nn.conv2d(input_layer, self.weights, strides=[1,self.stride,1,1], padding=self.padding, name=self.name)

      param_count = self.kernel_width * num_input_channels * self.num_kernels
...
class Pool1D:
   ...
   def build(self, input_layer):
      ...
      if self.pool_type == MAX_POOL:
         self.layer = tf.nn.max_pool(input_layer, ksize=[1,self.pool_width,1,1], strides=[1,self.stride,1,1], padding=self.padding, name=self.name)
      elif self.pool_type == AVG_POOL:
         self.layer = tf.nn.avg_pool(input_layer, ksize=[1,self.pool_width,1,1], strides=[1,self.stride,1,1], padding=self.padding, name=self.name)
      else:
...
class ReLU:
   ...
   def build(self, input_layer):
      ...
      self.bias = bias_variable(bias_shape, name='b_'+self.name)

      # Create the ReLU layer
      self.layer = tf.nn.relu(input_layer + self.bias, name=self.name)
...
class FullConnection:
   ...
   def build(self, input_layer):
      ...
      self.weights = weight_variable([input_size, self.output_size], name='w_'+self.name)

      # Create the ReLU layer
      self.layer = tf.matmul(input_layer, self.weights, name=self.name)

      return self.layer, input_size*self.output_size
...
class Flatten:
   ...
   def build(self, input_layer):
      ...
      # Create the ReLU layer
      self.layer = tf.reshape(input_layer, [-1, flattened_dimension.value], name=self.name)
...
class Softmax:
   ...
   def build(self, input_layer):
      ...
      self.weights = weight_variable([input_size, self.output_size], name='w_'+self.name)
      self.bias = bias_variable([self.output_size], name='b_'+self.name)
      self.layer = tf.nn.softmax(tf.matmul(input_layer, self.weights) + self.bias, name=self.name)

      param_count = input_size*self.output_size + self.output_size
